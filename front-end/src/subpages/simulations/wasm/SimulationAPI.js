export class SimulationAPI {
  ...
  static async create() {
    ...
    const module = await createModule({
      mainScriptUrlOrBlob: new URL("./main.js", import.meta.url).href,
    });
  ...
  simulationCaller(stock_prices, stock_weights, time, sims, cb) {
    ...
    try {
      ...
      stocks_ptr = this.module._malloc(
        stocks.length * stocks.BYTES_PER_ELEMENT,
      );
      weights_ptr = this.module._malloc(
        weights.length * weights.BYTES_PER_ELEMENT,
      );
      ...
      RESULTS = this.module.HEAPF64.subarray(
        res_ptr / RESULTS.BYTES_PER_ELEMENT,
        res_ptr / RESULTS.BYTES_PER_ELEMENT + RESULTS.length,
      ).slice();
    } catch (error) {
      console.error(error);
    } finally {
  ...
  runSimulationSerialized(stock_prices, stock_weights, time, sims) {
    return this.simulationCaller(
      stock_prices,
      stock_weights,
      time,
      sims,
      (...args) => this.module._runSimulations(...args),
    );
  }
  ...
  runSimulationMultithreading(stock_prices, stock_weights, time, sims) {
    return this.simulationCaller(
      ...
      time,
      sims,
      (...args) => this.module._runMultithreadingSimulations(...args),
    );
  }
}
