export function priceToLogReturns(priceSeries) {
  return priceSeries.map((prices) => {
    if (prices.length < 2) {
      throw new Error(
        "Each stock needs at least two prices to compute returns.",
      );
    }
    const returns = [];
...
export function runSimulation({
  ...
  if (!randomSampleProvider) {
    randomSampleProvider = (size) =>
      Array.from({ length: size }, gaussianRandom);
  }
  ...
  const outputs = Array.from({ length: sims }, () => ({
    ret: 0,
    stocksChange: Array(stocks).fill(0),
  }));
  ...
  for (let i = 0; i < sims; i++) {
    ...
    for (let t = 0; t < time; t++) {
      ...
      if (
        !Array.isArray(randomNormals) &&
        !(randomNormals instanceof Float64Array)
      ) {
        throw new Error(
          "Random sample provider must return an array-like vector.",
        );
      }
      if (randomNormals.length !== stocks) {
        throw new Error(
          `Random sample vector length ${randomNormals.length} does not match stocks ${stocks}.`,
        );
      }
      ...
      if (Number.isNaN(portfolioReturn)) {
        throw new Error(
          "Portfolio return became NaN during simulation. Check input prices and random sample provider.",
        );
      }
  ...
  returnsPercentiles.forEach((value, index) => {
    buffer[index] = value;
  });
  drawdownsPercentiles.forEach((value, index) => {
    buffer[9 + index] = value;
  });
  upsidesPercentiles.forEach((value, index) => {
    buffer[18 + index] = value;
  });
...
function createStockArray(stock_prices) {
  ...
  for (let i = 0; i < stocks_amount; i++) {
    ...
    if (curr_prices.length < days_back) {
      throw new Error(
        `Stock ${i} has only ${curr_prices.length} price points; expected ${days_back}.`,
      );
    }
...
function createWeightsArray(stock_weights) {
  var sum = stock_weights.reduce((a, b) => {
    return a + b;
  }, 0);
  var weights = new Float64Array(stock_weights.length);
  var new_weights = stock_weights.map((el) => {
    return el / sum;
  });
...
export function runSimulations(stocksFlat, weights, time, sims) {
  ...
  if (
    Array.isArray(stocksFlat) &&
    stocksFlat.length > 0 &&
    typeof stocksFlat[0] === "object"
  ) {
    ...
    throw new Error(
      "stocksFlat must be a numeric flat array or an array of stock objects.",
    );
  }
  ...
  if (typeof stocks_amount !== "number" || typeof stock_size !== "number") {
    throw new Error(
      "stocks_amount and stock_size must be provided as numbers.",
    );
  }
  ...
  if (stocksFlat.length !== stocks_amount * stock_size) {
    throw new Error(
      "stocksFlat length does not match stocks_amount * stock_size.",
    );
  }

  if (stock_size < 2)
    throw new Error(
      "Chunk size of stock prices should be at least two to count returns!",
    );
  if (stocks_amount <= 0)
    throw new Error("Analyzed stock amounts should be at least one!");
...
export class TestRandomGenerator {
  ...
  next(size) {
    ...
    if (value.length !== size) {
      throw new Error(
        `Request size ${size} did not match test sample length ${value.length}.`,
      );
    }
    return [...value];
  }
