...
test("validate empty ticker", () => {
  expect(validateAsset(ticker_list_dummy, "", 2, 4)).toBe(
    "Ticker should not be empty!",
  );
});

test("validate invalid ticker", () => {
  expect(validateAsset(ticker_list_dummy, "ZAB.WA", 2, 4)).toBe(
    "Please select ticker from the list.",
  );
});

test("validate quantity not a number", () => {
  expect(validateAsset(ticker_list_dummy, "LPP.WA", "test", 4)).toBe(
    "Quantity value should be a number.",
  );
});

test("validate price not a number", () => {
  expect(validateAsset(ticker_list_dummy, "LPP.WA", 4, "test")).toBe(
