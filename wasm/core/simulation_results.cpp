...
void CSimsResults::SetSimOutput(SimulationOutput& sims_output, const CMatrix<double>& weights) {
...
void CSimsResults::t_SetVAR(SimulationOutput& sims_output, CMatrix<double> weights) {
    ...
    std::vector<double> averages(weights.cols(), 0.0);
    ...
    while (sims_output.GetRet(idx) <= VAR) {
        ...
        auto stocks_change = sims_output.GetStocksChange(idx); // matrix of stocks
        // add averages turned into an exponential form
        for (int i = 0; i < averages.size(); i++) {
            averages[i] += std::exp(stocks_change[i]) - 1;
        }
        idx++;
    }
    ...
    for (int i = 0; i < averages.size(); i++) {
        averages[i] /= idx;
        averages[i] *= weights[0][i];
    }
    // multiply by weights to get CVaRs
    size_t buff_idx = m_cvar_ptr;
    for (int i = 0; i < averages.size(); i++) {
        t_buff[buff_idx + i] = averages[i];
    }
};

void CSimsResults::t_SetReturns(std::vector<double>& _rets) {
    auto res = t_CountPercentile(_rets);
    t_InsertToBuff(res, m_returns_ptr);
};

void CSimsResults::SetDrawdowns(std::vector<double>& _drawdowns) {
    auto res = t_CountPercentile(_drawdowns);
    t_InsertToBuff(res, m_drawdowns_ptr);
};

void CSimsResults::SetUpsides(std::vector<double>& _upsides) {
    auto res = t_CountPercentile(_upsides);
    t_InsertToBuff(res, m_upsides_ptr);
}

double CSimsResults::t_TransformToExp(double value) {
    return std::exp(value) - 1.0;
}

void CSimsResults::t_InsertToBuff(std::vector<double>& data, int start_ptr) {
...
std::vector<double> CSimsResults::t_CountPercentile(std::vector<double> &data) {
    if (data.size() == 0) {
